...
/**
 * Sample : CancelCheckoutAF Type : Attachment webservice
 * 
 * Description : This sample demonstrates the usage of 'CancelCheckoutAF'
 * webservice which is used in this sample to cancel the AF 'checked out'
 * status of a webservice.
 * 
 * Note:- testing user must be allowed to do CancelCheckout operation for AF.
 */
public class CancelCheckoutAF {

	private static final String COMMAND_NAME = "CancelCheckoutAF";
	...
	public static void main(String[] args) {
		...
		try {
			...
			CancelCheckOutAFRequestType cancelcheckoutAFRequestType = new CancelCheckOutAFRequestType();
			AgileCancelCheckOutAFRequest agilecancelCheckAFRequestType = new AgileCancelCheckOutAFRequest();
			...
			// number information
			// for the same.
			agilecancelCheckAFRequestType.setChangeNumber(changeNumber);
			agilecancelCheckAFRequestType.setFolderNumber(folderNumber);
			System.out.println("Cancel Check out a file folder " + folderNumber
					+ " in the Affected Files tab of the Change '" + changeNumber + "'...\n");

			cancelcheckoutAFRequestType.getRequests().add(agilecancelCheckAFRequestType);
			CancelCheckOutAFResponseType cancelcheckOutAFResponseType = agileStub
					.cancelCheckOutAF(cancelcheckoutAFRequestType);
			System.out.println("\nSTATUS CODE: " + cancelcheckOutAFResponseType.getStatusCode());

			// If the status code is not 'SUCCESS', then populate the list of
			// exceptions
			// returned by the webservice.
			if (!cancelcheckOutAFResponseType.getStatusCode().toString().equals(ResponseStatusCode.SUCCESS.value())) {

				RunAllSamples.reportFailure(clsName);

				AgileExceptionListType[] agileExceptionListType = cancelcheckOutAFResponseType.getExceptions()
				...
				AgileWarningListType agileWarningListType[] = cancelcheckOutAFResponseType.getWarnings()
				...
				System.out.print("The cancel checkout operation is done sucessfully ");
				System.out.println(agilecancelCheckAFRequestType.getChangeNumber() + ".");
			}
