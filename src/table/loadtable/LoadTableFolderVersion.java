 ...
 * of a table from a particular version of an Agile file folder. The request
 * object consists of specifications identifying the table and the file folder
 * along with an option 'folderVersion'. The response object consists of the
 *  rows of that table, with row content available as message elements.
 */
public class LoadTableFolderVersion {
    private static final String COMMAND_NAME = "LoadTableFolderVersion";
    ...
    public static void prepareData(String[] args){
        ...
        try{
            ...
            fileName = folderNumber + "_File124.txt";
            dataPrepare.checkOutFolder(folderNumber);
            dataPrepare.addAttachmentToFolder(folderNumber, new String[] {fileName} );
            dataPrepare.checkInFolder(folderNumber);
            
            folderVersion = dataPrepare.getValueFromTable("FileFolder", folderNumber, "TitleBlock", "version").toString();
            
            //dataPrepare.getFolderNameFromAttachment()
    ...
    public static void main(String[] args) {
        ...
        try{
            ...
            // Use the 'options' element to specify the particular folder version that
            // from which the specified table has to be loaded. Properties are defined
            ...
            properties[0].setPropertyName( SchemaConstants.FOLDER_VERSION.value() );
            properties[0].setPropertyValue( folderVersion );
            table[0].getOptions().addAll(Arrays.asList(properties));
            
            System.out.println("Loading the version " + folderVersion + " of the file folder '" + folderNumber + "'...\n");
