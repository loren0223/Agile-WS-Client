...
import com.agile.ws.schema.common.v1.jaxws.AgileWarningResolutionConstantsType;
...
import run.RunAllSamples;
 ...
 /**
 * Sample      : ApproveROWarningResolution
 ...
 * 
 * Additionally, the sample also demonstrates resolution of warnings in 
 * Agile webservices. The method of warning resolution used in this sample
 * may be applied equally effectively to other webservice scenarios.
 */
public class ApproveROWarningResolution {
    private static final String COMMAND_NAME = "ApproveROWarningResolution";
    ...
    public ApproveROWarningResolution() {
    ...
    public static void main(String[] args) {
        ...
        try{
            ...
            notifiers[0].setClassIdentifier(UserGroupConstants.CLASS_USER_GROUPS_CLASS.toString());
            notifiers[0].setObjectIdentifier("UG1");*/
            
            AgileUserUserGroupIdentifierType notifiers[] = new AgileUserUserGroupIdentifierType[1];
            notifiers[0] = new AgileUserUserGroupIdentifierType();                
            notifiers[0].setClassIdentifier( UserConstants.CLASS_USER.toString() );
            notifiers[0].setObjectIdentifier( user1 ); 
            
            agileApproveRObjectRequestType[0].getNotifiers().addAll(Arrays.asList(notifiers));

            agileApproveRObjectRequestType[0].setSignoffForSelf(true);
            
            // Agile Warnings are commonly encountered while executing webservices;
            // while these warnings do not cause a webservice failure by themselves, they 
            // need to be resolved in advance for the webservice to succesfully perform
            // the intended objectives.
            
            // This may be achieved by either disabling all warnings or by specifically
            // resolving a particular warning message. All warnings may be disabled by
            // setting the element: approveRObjectRequestType.getDisableAllWarnings(true)
            // A specific warning may be resolved by using AgileWarningResolutionType
            // and specifying the warning id. This warning id is obtained by using the
            // agileWarningType.getReference which returns a string value.
            
            AgileWarningResolutionType warningRes[] = new AgileWarningResolutionType[1];
            warningRes[0] = new AgileWarningResolutionType();
            
            // The following code disables a Collaboration warning that states that a
            // particular user has already signed off on the routable object, clarifying
            // whether the user wishes to sign off again. While a warning of this
            // type would not cause a webservice failure, the routable object would
            // not have been approved. To ensure the approval, the warning is
            // disabled as follows:
            warningRes[0].setId(182); // Obtain this warning Id from the response object.
            warningRes[0].setResolution(AgileWarningResolutionConstantsType.DISABLE);
            approveRObjectRequestType.getWarningResolution().addAll(Arrays.asList(warningRes));
